...
class HybridPQC:
    ...
    def _init_pqcdualusb(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                self.logger.info(f"   - Power Protection: {self.pqc_backend.power_protection_enabled}")
    ...
    def get_info(self) -> Dict[str, Any]:
        ...
        info: Dict[str, Any] = {
            ...
            'key_sizes': f'{len(self.pqc_public_key)}/{len(self.pqc_private_key)} bytes' if hasattr(self, 'pqc_public_key') else 'Not generated',
        ...
        if self.security_info:
            ...
            info['power_analysis_protection'] = self.security_info.get('power_analysis_protection', False)
        ...
        if self.pqc_backend is not None:
            try:
                ...
                info['power_protection_enabled'] = getattr(self.pqc_backend, 'power_protection_enabled', False)
                info['real_pqc_keys'] = self.pqc_available
                
                # Enhanced security status
                if self.pqc_available:
                    info['security_status'] = 'QUANTUM_RESISTANT'
                    info['note'] = f'Real {info["pqc_signature_algorithm"]} signatures with power analysis protection'
                else:
                    info['security_status'] = 'CLASSICAL_STRONG'
                    info['note'] = 'Classical RSA-4096 fallback (cryptographically strong but not quantum-resistant)'
