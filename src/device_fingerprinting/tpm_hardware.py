...
class TPMFingerprinter:
    ...
    def _get_windows_tpm(self) -> TPMInfo:
        ...
        try:
            ...
            return TPMInfo(
                available=False, platform="Windows", error="TPM not present or not enabled"
            )
    ...
    def _get_linux_tpm(self) -> TPMInfo:
        ...
        try:
            ...
            if not tpm_devices:
                return TPMInfo(
                    available=False, platform="Linux", error="No TPM device found in /sys/class/tpm"
                )
            ...
            try:
                result = subprocess.run(
                    ["tpm2_getcap", "properties-fixed"], capture_output=True, text=True, timeout=3
                )
